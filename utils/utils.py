...
import utils.tiou.rrc_evaluation_funcs as tiou
import utils.tiou.script as tiou_script
...
def LPIPS(out_image, gt_image, model):
    out_image = out_image - 1
    gt_image = gt_image - 1
    dist01 = model.forward(out_image, gt_image)
    return dist01
...
def TextDetectionLoss_DEBUG(out_image, gt_image, net, device):
    # out_image:  BxCxHxW
    out_image = torch.clamp(out_image, min=0, max=1)
    out_image = out_image * 255
    out_image = normalizeMeanVarianceTensor(out_image)
