...
class IC15TrainSet(Dataset):
    """ICDAR 15 Train dataset."""
    ...
    def __getitem__(self, idx):
        ...
        if self.use_cap:
            if self.cap_version == 1:
                ...
                txt_name = "gt_img_" + gt_img_name.replace(".jpg", ".txt")
        ...
        if self.ratio_multiplier > 0:
            input_patch = np.clip(
                np.array(input_patch).astype(float) * self.ratio_multiplier, 0.0, 255.0
            ).astype(np.uint8)
...
class IC15TestSet(Dataset):
    """ICDAR 15 Test dataset."""
    ...
    def __getitem__(self, idx):
        ...
        if self.ratio_multiplier > 0:
            input_img = np.clip(
                np.array(input_img).astype(float) * self.ratio_multiplier, 0.0, 255.0
            ).astype(np.uint8)
